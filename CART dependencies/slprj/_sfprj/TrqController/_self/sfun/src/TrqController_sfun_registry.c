...
unsigned int sf_process_check_sum_call( int nlhs, mxArray * plhs[], int nrhs,
  const mxArray * prhs[] )
{
  extern unsigned int sf_TrqController_process_check_sum_call( int nlhs, mxArray
    * plhs[], int nrhs, const mxArray * prhs[] );
  if (sf_TrqController_process_check_sum_call(nlhs,plhs,nrhs,prhs))
    return 1;
  return 0;
}
...
{
  extern unsigned int sf_TrqController_autoinheritance_info( int nlhs, mxArray *
    plhs[], int nrhs, const mxArray * prhs[] );
  if (sf_TrqController_autoinheritance_info(nlhs,plhs,nrhs,prhs))
    return 1;
  return 0;
}
...
unsigned int sf_process_get_eml_resolved_functions_info_call( int nlhs, mxArray *
  plhs[], int nrhs, const mxArray * prhs[] )
{
  extern unsigned int sf_TrqController_get_eml_resolved_functions_info( int nlhs,
  ...
  if (strcmp(machineName, "TrqController") == 0) {
    const mxArray *newRhs[2] = { NULL, NULL };

    newRhs[0] = prhs[0];
    newRhs[1] = prhs[2];
    return sf_TrqController_get_eml_resolved_functions_info(nlhs,plhs,2,newRhs);
  }
...
extern unsigned int sf_TrqController_method_dispatcher(SimStruct *S, unsigned
...
{
  if (!strcmp(machineName,"TrqController")) {
    return(sf_TrqController_method_dispatcher(simstructPtr,chartFileNumber,
            method,data));
  }
...
extern void TrqController_terminator(void);
...
extern void TrqController_initializer(void);
extern void TrqController_register_exported_symbols(SimStruct* S);
extern void TrqController_debug_initialize(void);
void sf_register_machine_exported_symbols(SimStruct* S)
{
  TrqController_register_exported_symbols(S);
}
...
void sf_machine_global_initializer(SimStruct* S)
{
  ...
  if (sfMachineGlobalInitializerCallable) {
    ...
    if (!simModeIsRTWGen) {
      TrqController_debug_initialize();
    }

    TrqController_initializer();
  }
